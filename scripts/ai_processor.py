...
你的任务：
1. 从中提取**最新的文章/更新条目**（通常在页面顶部或"最新"/"近期"/"changelog"区域）
2. **优先提取日期在 {date_range} 范围内的文章**
3. 如果文章有明确日期但不在目标范围内，不要提取（除非是2026年且接近目标范围）
4. 排除2025年及更早的旧文章、常青内容页面、资源页面、导航链接
5. 链接列表中已经包含了每篇文章的日期（date字段），优先使用这个日期
6. 对于 API changelog 类型的内容，将同一天的多个变更合并为一条，在摘要中列出所有变更点
7. **重要**：只提取真正的"更新/新闻/变更"，不要提取教程、指南、案例研究等常青内容

**关于日期的重要规则**：
- 如果能从页面明确提取到日期（如"March 18, 2026"或"2026-03-18"），使用该日期并设置 dateConfidence="high"
- 如果只能从标题或URL推断日期（如"2026-Q1-update"），设置 dateConfidence="medium"
- **如果完全无法确定日期，将 date 设为 null 并设置 dateConfidence="low"，不要猜测或使用当前日期**
- 绝对不要使用抓取日期、当前日期或目标周结束日期作为文章日期
...
{{
  ...
  "dateConfidence": "high/medium/low - 日期确定性（high=从页面明确提取，medium=从标题/URL推断，low=无法确定）"
}}
