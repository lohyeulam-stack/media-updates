...
4. 如果文章没有明确日期但出现在"最新更新"/"近期文章"区域或页面顶部位置，可以提取并推测日期
5. 排除2025年及更早的旧文章、常青内容页面、资源页面、导航链接
6. 链接列表中已经包含了每篇文章的日期（date字段），优先使用这个日期
7. 如果链接的date字段为空或"unknown"，尝试从文章标题、URL或文本内容中推断日期
8. 对于 API changelog 类型的内容，将同一天的多个变更合并为一条，在摘要中列出所有变更点
9. **重要**：只提取真正的"更新/新闻/变更"，不要提取教程、指南、案例研究等常青内容
