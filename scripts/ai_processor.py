...
2. **优先提取日期在 {date_range} 范围内的文章**，但如果文章没有明确日期或日期不确定，也可以提取（我们会在后续流程中过滤）
3. 排除2025年及更早的明确标注旧日期的文章
4. 链接列表中已经包含了每篇文章的日期（date字段），优先使用这个日期
5. 如果链接的date字段为空或"unknown"，尝试从文章标题、URL或文本内容中推断日期。如果完全无法确定日期，使用目标周的结束日期
6. 排除导航链接、页脚链接、广告链接、登录注册、"Learn more"/"Read more"按钮等非文章内容
7. 对于 API changelog 类型的内容，将同一天的多个变更合并为一条，在摘要中列出所有变更点
8. **重要**：如果页面上有最近的更新但日期不明确，宁可提取出来（标注推测日期），也不要遗漏重要信息
